...
class RuleBasedEventConverter:
    ...
    def convert(
        self,
        *rule_based_disturbance_paths: str | Path,
    ) -> tuple[pd.DataFrame, pd.DataFrame | None]:
        event_data = []
        transition_data = []
        for rule_based_disturbance_path in rule_based_disturbance_paths:
            events = load_csv(rule_based_disturbance_path).astype(
                {"event_id": "str"}
            )
        ...
        all_events = pd.concat(event_data)
        all_transitions = pd.concat(transition_data) if transition_data else None

        return all_events, all_transitions
